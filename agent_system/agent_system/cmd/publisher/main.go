...
func main() {
	...
	var rootCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			generator := generator.NewContentGenerator(ollamiClient, cfg.AgentsMdPath)
			topicSelector := selector.NewTopicSelector(cfg, postRepo, ollamaClient)
			gitPublisher := publisher.NewGitPublisher(".", cfg.GitRemote)
			...
			content, err := generator.GeneratePost(ctx, topic)
			if err != nil {
...
// Helper to avoid unresolvable error in type assertion if needed
func errorHelper() error {
	return nil
}
